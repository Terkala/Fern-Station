    ...
    private void UpdateBodyPartHighlight()
    {
        ...
        if (_selectedBodyPart.HasValue)
        {
            ...
            try
            {
                ...
                if (bodyPartLayer.HasValue && _spriteSystem.LayerMapTryGet((spriteEntity, sprite), bodyPartLayer.Value, out var bodyPartLayerIndex, false))
                {
                ...
                // Apply the same offset as the body part layer
                _spriteSystem.LayerSetOffset((spriteEntity, sprite), layerIndex, highlightOffset);
                
                // Note: SpriteView.Scale in XAML handles the scaling, so we don't need to scale the highlight layer
                // Red tint - try pure red first, if that doesn't work the format might be BGR or GRB
                // Using pure red: R=255, G=0, B=0 for maximum red intensity
                _spriteSystem.LayerSetColor((spriteEntity, sprite), layerIndex, new Color(255, 0, 0, 220));
